...
if (isset($_POST['username'])){
    ...
    if (mysqli_num_rows($connect->query($query))!=0){
        ...
        $email = $_POST['email'];
        $status = $_POST['status'];
        $connect->query("update member set username = '$username', fullname = '$fullname', 
                        phonenumber = '$phonenumber', address = '$address', email = '$email', 
                        status = '$status' WHERE id =" .$member['id']);

        header("Location: ?option=members");
    }
}
?> 
...
<section class="container col-md-6">
    <form method="post">
        ...
        <section>
            ...
            <a href="?option=members" class="btn btn-outline-primary">&lt;&lt;Back</a>
        </section>
    </form>
</section>